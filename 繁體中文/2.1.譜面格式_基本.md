...
攝影機的角度被三個東西控制：x角度，y角度，z角度（畢竟是在三維空間中）。
...
a changecamera angle 0 10 0 0 60 easeinousine
```
...
```
// 格式
a changecamera abs_angle 開始拍 執行拍 x角度 y角度 z角度 模式算式

// 示範（從第零拍開始，花十拍 將攝影機角度調整為（0, 0, 60））
a changecamera abs_angle 0 10 0 0 60 easeinousine
```

`絕對`角度的意思是，不管一開始在什麼角度，一定會變到指定的角度。換句話說，假設現在舞台是`182 30 79`，輸入`0 0 60`，最後的結果會是`0 0 60`

第三種，還是相對角度，但一次只動一個角度：
```
// 格式
a s_changecamera angle 項目（x/y/z） 開始拍 執行拍 新數值 模式算式

// 示範（從第零拍開始，花十拍 將攝影機角度調整為（原本的x + 180, 原本的y, 原本的z））
a s_changecamera angle x 0 10 180 easeinoutsine
```

第四種，用絕對角度，一次只動一個角度：
```
// 格式
a s_changecamera abs_angle 項目（x/y/z） 開始拍 執行拍 新數值 模式算式

// 示範（從第零拍開始，花十拍 將攝影機角度調整為（原本的x, 180, 原本的z））
a s_changecamera abs_angle y 10 10 180 easeinoutsine
```
...
// 這是okay的
a s_changecamera angle x 0 10 180 easeInOutSine
a s_changecamera angle y 0 10 60 easeOutSine
a s_changecamera angle z 5 5 180 linear
```
...
// 不可以同時動x！（y跟z也不行！）
a s_changecamera angle x 0 10 180 easeInOutSine
a s_changecamera angle x 0 10 60 easeOutSine
a s_changecamera angle x 5 5 180 linear
```
...
// 示範（從第零拍開始，花十拍 將攝影機角度調整為（原本的x, 原本的y, 原本的z + 5））
a changecamera position 0 10 0 0 5 easeinouesine


// 絕對位置，三項目
a changecamera abs_position 開始拍 執行拍 x位置 y位置 z位置 模式算式

// 示範（從第10拍開始，花4拍 將攝影機角度調整為（2, 1, 4））
a changecamera abs_position 10 4 2 1 4 easeInOutSine


// 相對位置，單項目
a s_changecamera position 項目(x, y, z) 開始拍 執行拍 新數值 模式算式

// 示範（從第4拍開始，花4拍 將攝影機x角度調整為 原本的x - 4））
a s_changecamera position x 4 4 -4 linear

// 絕對位置，單項目
a s_changecamera abs_position 開始拍 執行拍 x位置 y位置 z位置 模式算式

// 示範（從第2拍開始，花5拍 將攝影機z角度調整為2））
a s_changecamera position z 2 5 2 easeInCubic
```
### 示範
```
