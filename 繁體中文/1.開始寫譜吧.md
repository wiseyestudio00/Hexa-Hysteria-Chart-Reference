...
如果想要使用iOS的裝置來創作譜面，可以使用這個方式。<br>
您可以在Hexa Hysteria的Discord群組，`譜師/重要資源`裡找到Hexa Hysteria的Public Link。<br>
...
##### Windows下載方式
1. 從雲端下載檔案
2. 打開遊戲，進入`開發者介面`，點擊`open folder`
3. 會有一個檔案夾跳出來，把下載好的檔案拖進去。

取代完成後，就能開始寫譜了

### 3. 推薦的寫譜的方法
#### 1. 推薦的iOS文字編輯器
（這裡是為了iOS的使用者而寫的。如果您使用的是其他版本，可以跳過。）

1. 在App Store上，下載免費軟體[Pretext](https://apps.apple.com/tw/app/pretext/id1347707000)
2. 這是文字編輯器，讓你能在iOS上編輯文字
3. 在Pretext裡，選擇`我的裝置/Hexa Hysteria/TestChart`裡，建立新文檔。名稱為chart，副檔名為.txt。
4. 開啟後就能製作譜面了

#### 2. 推薦的Windows文字編輯器
基本上，預設的就已經可以使用了。如果想要其他的文字編輯器的話，可以參考Atom：https://atom.io/

1. 在從遊戲內打開的檔案夾中，創造一個chart.txt
2. 直接編輯chart.txt即可
